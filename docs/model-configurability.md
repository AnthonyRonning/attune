...
GEPA rollout infrastructure errors are not optimization examples. Target-model API envelope failures such as transport errors, HTTP status errors, and response-decode errors are retried and then stop the run with a best-so-far checkpoint instead of becoming `0.0` rollout scores. Judge transport failures, non-JSON judge output, and invalid judge JSON also abort the run rather than being normalized into a score. That keeps provider, account, sandbox, and judge failures from producing empty or misleading "best" instructions. Successful target-model API responses with empty or malformed DSRs content still remain valid scoreable model-behavior failures and should be curated into model/profile-specific datasets when they represent real behavior. `ATTUNE_GEPA_DEBUG=1` enables per-case rollout diagnostics for debugging this boundary.
...
The latest promoted Qwen request-adapter line used OpenRouter `anthropic/claude-sonnet-5` for reflection/proposal and judging, OpenRouter target calls, hidden labels outside reflected examples, and the previous reviewed artifact as `--seed-artifact`. Earlier promoted Qwen and Gemma lines used native Anthropic Claude Sonnet 4.6 for reflection/proposal and judging. New GEPA runs should use the current default, OpenRouter `anthropic/claude-sonnet-5`, unless a direct-Anthropic comparison is intentional. Current promotion policy treats long and behavior-specific instructions as acceptable when the corrected GEPA workflow and artifact review support them; the main rejection criteria are hidden-label leakage, exact expected-output leakage, trace-specific answers, and brittle one-off rules.

The earlier Qwen regenerated-context artifact scored `0.8450` on the smaller post-50 dataset, but that score is no longer used as the default decision point. The larger curated Qwen dataset exposed regressions during baseline comparison, so the current promoted Qwen default is the best regenerated-context candidate from the shuffled Qwen follow-up run.
...
| `qwen/qwen3.5-9b` | `regenerated_context` | `0.6905` | previously promoted as Qwen built-in default; follow-up 500-trace proxy replay improved from 457/500 to 471/500 |
| `qwen/qwen3.5-9b` | `regenerated_context` | `0.8205` | promoted as Qwen built-in default; follow-up 500-trace proxy replay improved from 471/500 to 475/500 |
| `qwen/qwen3.5-9b` | `append_only` | `0.7614` | not promoted; follow-up 500-trace proxy replay reached 469/500 and required more correction-agent attempts than regenerated-context |
| `google/gemma-4-26b-a4b-it` | `append_only` | `0.9273` | previously promoted as Gemma built-in default; follow-up 500-trace proxy replay reached 497/500 |
| `google/gemma-4-26b-a4b-it` | `regenerated_context` | `0.9227` | not promoted; close, but append-only still won |
| `google/gemma-4-26b-a4b-it` | `append_only` | `0.9650` | promoted as Gemma built-in default; follow-up 500-trace proxy replay reached 498/500 |
| `google/gemma-4-26b-a4b-it` | `regenerated_context` | `0.9639` | not promoted; follow-up 500-trace proxy replay reached 497/500 with fewer correction attempts |

The active built-in Qwen artifact is `datasets/request-adapter/qwen-dsrs-sonnet5-shuffled-r1-regenerated-context-gepa.json` with `dsrs_history_format = "regenerated_context"`. A follow-up proxy-only replay of the same 500-scenario file improved Qwen from the previous promoted proxy result of 471/500 structural passes to 475/500, while the prior direct baseline from the side-by-side comparison remains 487/500. Valid DSR `tool_calls` converted into OpenAI `tool_calls` are expected adapter behavior, not correction-agent repair; the promoted replay had 397/500 direct valid outputs, 99 correction-agent attempts, and 70 accepted correction recoveries. The same Sonnet-5 loop also produced an append-only artifact, but its replay reached 469/500 with 131 target structural failures before correction and 130 correction-agent attempts, so it remains an alternate experiment artifact rather than a default. The active built-in Gemma artifact is `datasets/request-adapter/gemma-dsrs-conservative-sonnet5-shuffled-r1-append-only-gepa.json` with `dsrs_history_format = "append_only"`. Its 500-scenario replay reached 498/500; the regenerated-context candidate was cleaner at the target layer with 7 correction attempts instead of 16, but reached 497/500, so append-only remains the default. Earlier fresh, post-50, and Qwen-500 Sonnet artifacts remain checked in as reviewed alternates and provenance so future comparisons do not depend on ignored local experiment files.
...
Correction-agent GEPA can now filter the shared correction dataset with
`--dataset-model` and `--dataset-profile` before optimization. Use those flags
for per-model correction artifacts, then gate promotion with
`eval-correction-agent` against the same filtered rows. The correction-only eval
enforces `possible=true` and non-empty expected `content` in addition to
normalized tool-call equality, so malformed correction-agent envelopes are not
hidden by otherwise-correct tool calls.
