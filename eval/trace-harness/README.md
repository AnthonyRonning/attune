...
This starts the proxy in-process unless `--proxy-url` is provided. It reads `OPENROUTER_API_KEY` or `MCP_UPSTREAM_API_KEY` from the environment or `.env`.

```bash
cargo run -- trace-harness run \
  --scenarios-path eval/trace-harness/scenarios/pi-mono.local.jsonl \
