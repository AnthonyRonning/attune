...
pub struct GEPA {
    ...
    /// Seed for deterministic epoch-shuffled minibatch sampling
    #[builder(default = 0)]
    pub seed: u64,
...
#[derive(Debug, Clone)]
struct EpochShuffledMinibatchSampler {
    dataset_len: usize,
    batch_size: usize,
    seed: u64,
    epoch: u64,
...
impl EpochShuffledMinibatchSampler {
    fn new(dataset_len: usize, minibatch_size: usize, seed: u64) -> Self {
        let batch_size = minibatch_size.clamp(1, dataset_len.max(1));
        let order = shuffled_indices(dataset_len, seed, 0);
        Self {
            dataset_len,
            batch_size,
            seed,
    ...
    fn next_batch(&mut self) -> Vec<usize> {
        ...
        while batch.len() < self.batch_size {
            if self.cursor >= self.order.len() {
                ...
                self.order = shuffled_indices(self.dataset_len, self.seed, self.epoch);
                self.cursor = 0;
            }

            let remaining_epoch = self.order.len() - self.cursor;
            let remaining_batch = self.batch_size - batch.len();
            let take = remaining_epoch.min(remaining_batch);
...
fn shuffled_indices(dataset_len: usize, seed: u64, epoch: u64) -> Vec<usize> {
    let mut indices = (0..dataset_len).collect::<Vec<_>>();
...
impl GEPA {
        ...
        M: Module + Optimizable + FeedbackEvaluator,
    {
        ...
        for generation in 0..self.num_iterations {
            ...
            // Sample minibatch using epoch-shuffled coverage.
            let minibatch: Vec<Example> = minibatch_sampler
                .next_batch()
                .into_iter()
...
mod tests {
    ...
    fn epoch_shuffled_minibatches_cover_every_example_before_repeating() {
        let mut sampler = EpochShuffledMinibatchSampler::new(5, 2, 7);

        let first = sampler.next_batch();
        let second = sampler.next_batch();
        let third = sampler.next_batch();

        let mut first_epoch = first
            .iter()
            .chain(second.iter())
    ...
    #[test]
    fn epoch_shuffled_minibatches_are_seed_deterministic() {
        let mut left = EpochShuffledMinibatchSampler::new(8, 3, 42);
        let mut right = EpochShuffledMinibatchSampler::new(8, 3, 42);

        for _ in 0..5 {
            assert_eq!(left.next_batch(), right.next_batch());
        }
    }
}
